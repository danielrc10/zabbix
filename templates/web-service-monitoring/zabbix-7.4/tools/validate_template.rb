...
dashboards.each do |dashboard|
  ...
  card_fields = fields.to_h { |field| [field.fetch('name'), field.fetch('value')] }
  check(card_fields['tag_agrupamento'] == 'site', 'Dynamic Status Cards must group website items by the site tag')
  check(!card_fields.key?('linhas'), 'legacy JSON row configuration must not be used')

  card_rows_by_index = Hash.new { |hash, key| hash[key] = {} }
  fields.each do |field|
    match = field.fetch('name').match(/\Alinhas\.(\d+)\.([^.]+)(?:\.(\d+))?\z/)
    next unless match

    row = card_rows_by_index[match[1].to_i]
    if match[3]
      row[match[2]] ||= []
      row[match[2]][match[3].to_i] = field.fetch('value')
    else
      row[match[2]] = field.fetch('value')
    end
  end
  card_rows = card_rows_by_index.sort.map(&:last)
  check(card_rows.length == 6, 'website card must contain six configured rows')
  http_row = card_rows.find { |row| row['rotulo'] == 'HTTP' }
  check(http_row&.fetch('estado_modo', nil) == 'valores', 'HTTP card must use exact-value evaluation')
  check(http_row&.fetch('valores_ok', nil) == '200', 'HTTP 200 card state must be OK')
  check(http_row&.fetch('estado_padrao', nil) == 'critico', 'unexpected HTTP card state must be critical')
  expiration_row = card_rows.find { |row| row['rotulo'] == 'Expira em' }
  check(expiration_row&.fetch('padroes_estado', []) == ['[*] Certificate: Health'],
    'expiration-date row must inherit certificate health')
  check(expiration_row&.fetch('valores_ok', nil) == '2', 'expiration-date OK mapping is invalid')
  check(expiration_row&.fetch('valores_aviso', nil) == '1', 'expiration-date warning mapping is invalid')
  check(expiration_row&.fetch('valores_critico', nil) == '0', 'expiration-date critical mapping is invalid')
  validity_row = card_rows.find { |row| row['rotulo'] == 'Validade' }
  check(validity_row&.fetch('padroes_estado', []) == ['[*] Certificate: Health'],
