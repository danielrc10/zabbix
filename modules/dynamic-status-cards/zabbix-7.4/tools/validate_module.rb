...
check(combined_php.include?("EXIBICAO_RESUMO_HISTORICO"), 'text-only historical summary mode is missing')
check(combined_php.include?("AGREGACAO_HISTORICA_SOMA"), 'historical sample sum is missing')
check(combined_php.include?("AGREGACAO_HISTORICA_MEDIA"), 'historical sample average is missing')
...
check(combined_php.include?("dynamic-status-card__conteudo"), 'scalable card-content wrapper is missing')
...
check(stylesheet.include?('repeat(var(--dsc-rows), minmax(0, 1fr))'),
  'runtime full-height card rows are missing')
check(stylesheet.include?('scale(var(--dsc-content-scale))'), 'continuous inner-content scaling is missing')
check(stylesheet.include?('overflow-y: hidden'), 'vertical widget scrolling must be disabled')
check(!stylesheet.include?('overflow-y: auto'), 'vertical widget scrolling must not be re-enabled')
...
check(controller.include?("style.setProperty('--dsc-rows'"), 'runtime row update is missing')
check(controller.include?("style.setProperty('--dsc-content-scale'"), 'continuous content scale update is missing')
check(controller.include?('#selectLayout'), 'two-dimensional layout selection is missing')
check(controller.include?('#isBetterLayout'), 'layout quality comparison is missing')
check(controller.include?('#getViewportSize'), 'actual widget rectangle measurement is missing')
check(controller.include?('content.scrollHeight'), 'card content height measurement is missing')
